...
module ov7670_config(
    ...
    //--------------------------------------------------------------------------
    // Configuration table — Full VGA (640x480) / RGB565 init
    // เปลี่ยนการตั้งค่าจาก QVGA เป็น VGA แบบเต็มจอ
    //--------------------------------------------------------------------------
    reg [15:0] config_regs [0:29];

    initial begin
        config_regs[ 0] = 16'h12_80;  // COM7  : soft reset (รอ 2 ms หลังคำสั่งนี้)
        config_regs[ 1] = 16'h12_04;  // COM7  : VGA (0x00) + RGB output (0x04) *** เปลี่ยนจาก 14 เป็น 04
        config_regs[ 2] = 16'h11_00;  // CLKRC : external clock directly
        config_regs[ 3] = 16'h0C_00;  // COM3  : Disable Scaling (ปิดระบบย่อภาพ) ***
        config_regs[ 4] = 16'h3E_00;  // COM14 : default (no manual scaling)
        ...
        // --- ค่า Windowing สำหรับ Full VGA (640x480) ---
        ...
        config_regs[22] = 16'h1A_7A;  // VSTOP
